/* FamilyDashBoard ServiceWorker — v14.29.0
...
// SyncEvent is part of the Background Sync spec — not yet in TS standard lib.
...
const CACHE_NAME = `familydashboard-v${__APP_VERSION__}`;
const CACHE_NAME_API = `familydashboard-api-v${__APP_VERSION__}`;
 ...
 */
async function _loadPrecacheManifest(): Promise<string[]> {
  try {
    ...
    return Array.isArray(data) && data.length > 0 ? (data as string[]) : APP_SHELL;
  } catch {
    return APP_SHELL;
  }
}
...
const API_CACHE_ORIGINS: string[] = [
  "api.open-meteo.com",
  "www.hebcal.com",
  "open.er-api.com",
  "api.frankfurter.dev",
...
// canonical source (previously mirrored from src/core/sw-constants.ts,
...
function _ttlForOrigin(hostname: string): number {
...
const OFFLINE_HTML =
  ...
  `<title>לוח משפחתי - מצב לא מקוון</title><style>body{background:#060b14;color:#e2e8f0;` +
  `display:flex;align-items:center;justify-content:center;height:100vh;margin:0;font-family:system-ui}` +
  `</style></head><body><div style="text-align:center"><div style="font-size:4em">📺</div>` +
  `<h1>לוח משפחתי</h1><p>אין חיבור לאינטרנט. הנתונים יוצגו ברגע שהחיבור יחזור.</p>` +
  `</div></body></html>`;
  ...
  event.respondWith(
    caches.open(CACHE_NAME).then((cache) =>
      cache.match(event.request).then((cached) => {
        const fresh: Promise<Response | null> = fetch(event.request)
          .then((response) => {
 ...
 */
async function _queueErrorReport(payload: unknown): Promise<void> {
  try {
 ...
 */
async function _flushErrorQueue(): Promise<void> {
  let cache: Cache;
  try {
