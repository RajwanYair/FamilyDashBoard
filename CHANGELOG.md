...
- 🔐 **fix(worker)**: complete S08-02 provider boundary hardening — reject URL credentials, non-default ports, unsafe calendar suffix hosts, and redirects; bound news/ICS/R2 response bodies; preserve calendar stale fallback; and add regression coverage for rejected inputs and read failures
- 🛠️ **ops(worker)**: complete S08-03 operational controls — document request/storage/egress/inference budgets and shutdown signals, bound synthetic provider probing with KV/memory caching, prune route-hit telemetry after 30 days, and align AI synthesis caching with its four-hour contract
...
- ⚡ **perf(cls)**: P5 explicit img dimensions on stock logo, contain: layout on card\_\_body (S49)
