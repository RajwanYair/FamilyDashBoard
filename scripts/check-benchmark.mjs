...
const TOTAL_BUDGET_GROWTH = 0.8; // Total duration can grow max 80% from baseline (warm-state CI)
...
function checkRegression(data, baseline) {
  ...
  for (const file of data.files) {
    ...
    if (file.duration > allowed && file.duration - baselineMs > 4000) {
      // Only flag if delta > 4000ms (noise filter — prevents false positives
