...
- **S00-R2 - Worker/browser restoration (Blocked; 1 day).** Anchor: [Worker manifest](../worker/package.json), [Worker lockfile](../worker/package-lock.json), [CI installer](../.github/ci/install-tools.sh), [Playwright config](../playwright.config.ts). Hosted CI validates locked Worker installation/typecheck, production build, Lighthouse, and Chromium smoke/accessibility. Locally, Worker dependency directories are empty; `npm run typecheck --prefix worker` fails with TS2688 and `npm ci --ignore-scripts --prefix worker` exits 1 under Node 26.8.1. Issue [#145](https://github.com/RajwanYair/FamilyDashBoard/issues/145) tracks restoration. Acceptance remains a normal lockfile install without force/legacy peers, successful required imports, and actual launches of the configured browsers.
