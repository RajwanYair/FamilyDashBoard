...
| Browser and coverage testing  | Shared Playwright 1.60.0; Vitest browser and coverage-v8 4.1.7; Chromium metadata resolves, but the required headless-shell executable is absent        | Restore the matching Chromium browser binary, then rerun smoke and accessibility projects                                      |
...
**Status**: In progress. **Evidence**: package resolution and exact CLI checks passed for TypeScript 6.0.3 and Vitest 4.1.7; `tests/unit/core/freshness.test.ts` was runnable; `.vscode/tasks.json` now passes Prettier parsing; VS Code settings, tasks, extensions, and Copilot configuration now report no diagnostics; repository and Copilot setup Node declarations now match the CI Node 24 line; Playwright 1.60.0 was invoked through the Chromium smoke suite and reported the missing `chromium_headless_shell-1223` executable. **Remaining**: restore the matching browser binary, restore Worker-local imports, complete trusted update/advisory review, and capture a complete quality-gate exit. **Validation**: package import probes, exact CLI versions, focused unit smoke, browser launch, extension diagnostics, and setup walkthrough. **Rollback**: restore approved shared package versions; do not move or delete either checkout.
...
**2026-09-08 readiness evidence**: Playwright Chromium, headless shell, FFmpeg, and Winldd v1223 are installed; the Chromium smoke suite passed all 6 tests. The local runtime remains blocked because `node` is not resolvable from the current PowerShell PATH while npm resolves through the shared toolchain, and Playwright emits local Node 26 `DEP0205`. Hosted CI proves Worker installation and typecheck; local Worker import resolution and the remaining configured browser engines still need validation.

**2026-09-08 accessibility evidence**: Chromium axe found a real `aria-prohibited-attr` defect in the countdown upcoming-holidays container and weather precipitation/forecast containers. The affected labeled containers now use explicit semantic roles, and the focused countdown unit suite passed 132 tests; Chromium TV and tablet axe runs report zero critical/serious violations. Full configured browser coverage, manual keyboard/focus/screen-reader checks, and the editor's broader pre-existing static ARIA warnings remain open, so S05 is not complete.

- **S00-R1 - Supported local runtime (Blocked; 0.5-1 day).** Anchor: [.nvmrc](../.nvmrc), [local development](local-dev.md), [workspace tasks](../.vscode/tasks.json). Confirm a supported Node 24 executable and shared package resolution from a fresh PowerShell shell. Preserve global installs and report any required operator installation. Acceptance: core CLI/import probes and focused tests exit 0 without local runtime deprecations; workspace task and terminal use the same executable. Rollback: restore the prior approved path, not an untracked machine-wide upgrade.
- **S00-R2 - Worker/browser restoration (In progress; 1 day).** Anchor: [Worker manifest](../worker/package.json), [Worker lockfile](../worker/package-lock.json), [CI installer](../.github/ci/install-tools.sh), [Playwright config](../playwright.config.ts). Validate locked Wrangler/types peers, download matching browsers, run Worker typecheck and Chromium smoke, then configured browser engines. Acceptance: normal `npm ci --ignore-scripts` without force/legacy peers; required imports and actual browser launches pass. Local registry failure is still an explicit blocker.
...
- **S01-R1 - Shell and test determinism (In progress; 1 day).** Anchor: [tasks](../.vscode/tasks.json), [Vitest config](../vitest.config.ts), [video tests](../tests/unit/cards/video-news.test.ts). Verify Windows shell selection, simulated-frame network isolation, custom-element side-effect imports, and cleanup. Add a no-live-network regression fixture without hiding unexpected console errors. Acceptance: focused suite and two full-suite executions finish with current assertions intact and no external iframe requests; benchmark baseline is unchanged.
- **S01-R2 - Install scope and audit accuracy (Planned; 1-2 days).** Anchor: [installer](../.github/ci/install-tools.sh), [security workflow](../.github/workflows/security.yml), [CI](../.github/workflows/ci.yml). Inventory consumers before splitting tool groups; isolate performance-only dependencies from ordinary jobs without deleting their checks. Audit actual Worker and tooling trees, not an empty root manifest. Acceptance: every used CLI is declared, package resolution is repeatable, advisories/deprecations are owned and fixed, and security results reflect the installed scope.
- **S01-R3 - Gate failure fixtures (Planned; 1 day).** Anchor: [benchmark checker](../scripts/check-benchmark.mjs), [CI](../.github/workflows/ci.yml), [release tests](../tests/unit/scripts/release-workflow.test.ts). Inject one failure at a time for typecheck, lint, tests, benchmark, bundle, and signing-input completeness in an isolated fixture. Acceptance: nonzero exit stops downstream publication; all positive fixtures pass; missing or cancelled results never count as success.
...
- **S05-R1 - Input and status comprehension (1-2 days).** Anchor: [E2E](../tests/e2e), [accessibility styles](../src/styles/a11y.css). Extend focus-return, hidden-card, text-zoom, reduced-motion, keyboard/remote and touch scenarios. Acceptance: zero serious/critical axe findings, manual focus and screen-reader checklist completed, no color-only stale indicators or repeated announcements. Unavailable physical browsers remain explicit gaps.
...
- **S06-R1 - Display endurance (2 days plus 72-hour soak).** Anchor: [performance](../src/core/perf.ts), [idle scheduling](../src/core/idle.ts), [bundle checker](../scripts/check-bundle-size.mjs). Record baseline CPU, memory, requests and layout shifts; inject outages, channel changes and midnight rollover. Acceptance: existing bundle limits pass, retries/listeners remain bounded, retained-heap growth above 10% is investigated, and post-soak controls work. Do not substitute a short CI run for the soak.
- **S09-R1 - Feed relevance without loss (2 days).** Anchor: [news](../src/cards/news/news.ts), [SimHash tests](../tests/unit/core/simhash.test.ts). Build a synthetic Hebrew/mixed-language duplicate corpus, missing/future publication dates and stable tie-break cases; compare chronological and ranked results. Acceptance: materially different updates remain visible, ranking is deterministic, provenance and age are accessible, and read/starred IDs survive reload without collisions.
...
- **S07-R2 - Reproducibility and rollback drill (2 days).** Anchor: [reproducibility checker](../scripts/check-reproducible.mjs), [deployment guide](deployment.md). Build twice from identical declared inputs, compare output digests, verify artifact signatures with exact repository/workflow identity, and rehearse restoration of prior artifacts. Acceptance: tampered/missing/wrong-identity artifacts fail verification; update/rollback preserves settings; dry-run manifests never qualify as reproducibility proof.
