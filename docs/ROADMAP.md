...
> **Refresh date**: 2026-05-18 · **Shipped baseline**: v14.33.0 · **Active stream**: V15-OPEN.
>
> **v6.0 audit stamp (2025-07-14)**: v14.33.0 sprint batch (S11-S19). AlertsLiveDO Hibernatable WebSocket fan-out shipped (ADR-089) — replaces SSE AlertsOrchestrator with zero idle-CPU Durable Object, global broadcast, EU jurisdiction, `/api/alerts/live` WS route. R2 asset proxy route shipped (ADR-050 R2 leg) — `/api/r2-asset` serves background images from R2 with `X-Cache: HIT/MISS`, 10-origin allowlist, SHA-256 keying. Vectorize semantic dedup shadow mode launched (ADR-090, ADR-052) — `vectorize-client.ts` helper, `fdb-news-dedup` 384-dim cosine index wired into news aggregate handler as fire-and-forget shadow run (gated `VECTORIZE_INDEX+AI`). Web Push VAPID skeleton shipped (ADR-091) — 4 routes (`/api/push/key`, `POST/DELETE /subscribe`, `POST /send`), KV-backed subscription store, token-gated send, 501 until VAPID keys provisioned. ADR-089/090/091 documented. ADR index regenerated (91 ADRs). All worker types clean. 16+16+13 new unit tests.
>
...
>
> **Inventory**: 7765+ tests / 326 suites / 0 failures · 0 lint errors · 0 lint warnings · 0 `eslint-disable` · 0 `@ts-ignore` · 91 ADRs · 0 client deps · 2 worker deps (Hono + Valibot) · 7 themes · 12 cards · 4-tier offline cache · Worker ≤ 75 KB gzip · LHCI perf `error 0.99` · SLSA L2 + Sigstore + rebuilder manifest.
