...
8. Tool upgrades must preserve both supported installation modes. Pin repository development tools in the local manifest and lockfile for standalone reproducibility, keep the shared `MyScripts` installation available for sibling workspaces, and do not hide a broad machine upgrade or repository relocation inside a sprint.
...
| Lint and formatting           | Shared ESLint 9.39.1; Prettier 3.8.3; Stylelint 17.12.0; markdownlint-cli2 0.22.1                                                                                          | Confirm plugins and project configuration resolve, not merely CLI presence                                                     |
...
| Worker and fast lint          | Worker lockfile is present and hosted Worker tests/typecheck pass, but the local `worker/node_modules` tree is incomplete (`@cloudflare/workers-types` and `tsc` are unavailable); root dependencies are also absent | Restore Worker-local and root dependencies from the approved registry, then rerun local typecheck and focused tests |
...
**Status**: In progress. **Evidence**: the approved shared `MyScripts` install resolves TypeScript 6.0.3 and the focused Vitest toolchain from a fresh PowerShell shell. The active local runtime is Node 26.8.1, while the supported roadmap target is Node 24; no Node 24 binary or version manager is currently installed. The Worker package directories exist but are empty, so Worker typecheck fails with TS2688; a lockfile install also exits 1 under the unsupported runtime. Issue [#145](https://github.com/RajwanYair/FamilyDashBoard/issues/145) tracks restoration. Prior hosted evidence includes TypeScript 6.0.3, Vitest 4.1.7, Chromium smoke/accessibility runs, and CI Worker installation/typecheck. **Remaining**: restore Node 24, reinstall Worker dependencies from the committed lockfile, validate Worker-local imports and all configured browsers, complete trusted update/advisory review, and capture a complete quality-gate exit. **Validation**: fresh-shell shared-toolchain resolution passed; focused S01 regression passed 152 tests; Worker typecheck and full-gate completion remain blocked. **Rollback**: restore approved shared package versions; do not move or delete either checkout.
...
- **S00-R1 - Supported local runtime (In progress; 0.5-1 day).** Anchor: [.nvmrc](../.nvmrc), [local development](local-dev.md), [workspace tasks](../.vscode/tasks.json). The approved shared install at `C:\Users\ryair\OneDrive - Intel Corporation\Documents\MyScripts\node_modules` resolves from this checkout through the ignored local junction, and a fresh PowerShell shell resolves TypeScript 6.0.3. The active runtime is Node 26.8.1 rather than the supported Node 24 line, so the acceptance is not complete. Remaining: restore Node 24 and rerun the package-import and focused-unit checks from a fresh shell. Rollback: remove the local junction and restore the approved shared path, not an untracked machine-wide upgrade.
- **S00-R2 - Worker/browser restoration (Blocked; 1 day).** Anchor: [Worker manifest](../worker/package.json), [Worker lockfile](../worker/package-lock.json), [CI installer](../.github/ci/install-tools.sh), [Playwright config](../playwright.config.ts). Hosted CI validates locked Worker installation/typecheck, production build, Lighthouse, and Chromium smoke/accessibility. Locally, the Worker tree is present but incomplete: `@cloudflare/workers-types` and the Worker-local `tsc` entry point are unavailable, so `npm run typecheck --prefix worker` cannot run successfully; root `node_modules` and `package-lock.json` are also absent. Issue [#145](https://github.com/RajwanYair/FamilyDashBoard/issues/145) tracks restoration. Acceptance remains a normal lockfile install without force/legacy peers, successful local imports, and actual launches of the configured browsers.
