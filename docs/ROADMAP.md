...
Checks on 2026-09-10. Versions below are installed observations, **not a claim that they are latest**. The current checkout is `C:/GitHub/FamilyDashBoard.worktrees/roadmap-iteration-vibe-coding`; its root and Worker dependencies resolve from the restored local lockfiles. Preserve the checkout and lockfiles; do not substitute an unreviewed global install.

| Tool or group                 | Observed state                                                                                                                                                | Required action                                                                                                                |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Node.js / npm                 | Node 24.21.0; npm 11.19.0; repo [.nvmrc](../.nvmrc) and Copilot setup target Node 24                                                                          | Keep local and CI declarations on the supported Node 24 line; do not upgrade majors without a compatibility review             |
...
| TypeScript / Vite / Vitest    | Root install: 6.0.3 / 8.1.5 / 4.1.7; Worker-local typecheck passes with its lockfile                                                                          | Keep root and Worker lockfiles authoritative; review updates as a tested group                                                 |
| Browser and coverage testing  | Playwright 1.60.0 with Chromium available; six Chromium smoke tests pass locally; other configured browser engines and manual device checks remain unverified | Install or validate only the required engines through trusted channels; retain manual device checks as explicit evidence gaps  |
...
| Worker and fast lint          | Worker lockfile and local dependencies are restored; `npm --prefix worker run typecheck` passes; the root ESLint config intentionally ignores `worker/src/**` | Keep Worker typecheck as the validated Worker gate; add Worker lint only with a reviewed project configuration                 |
...
| Documentation checks          | Installed markdownlint-cli2 and Prettier pass on the roadmap, privacy, security, and data-source documents                                                    | Keep documentation checks on the installed lockfile toolchain; do not use registry-resolving `npx` fallbacks                   |

**Readiness decision: PARTIALLY UNBLOCKED for core development.** On 2026-09-10, this checkout resolved the pinned root and Worker toolchains, Worker typecheck passed, the focused privacy/provider suite passed, and all six Chromium smoke tests launched and passed through Playwright 1.60.0. Trusted update/advisory status, the complete configured-browser matrix, manual device evidence, and a clean full-gate completion remain unverified; S00 is therefore still not Done.
...
**Status**: In progress. **Evidence**: CI lint includes the repository CSS gate; the CI installer fails during setup if any core CLI entry point is unavailable; the 32 VS Code tasks reference valid package scripts; the Worker audit probe now propagates its native exit code instead of masking failure; the canonical `check` uses `check:benchmark` as its single full-suite test gate; the Windows benchmark launcher invokes npm through Node 24 instead of the rejected `.cmd` shim; the refreshed baseline covers 341 files and 8016 tests; `npm run check:benchmark` passes; the S01 benchmark regression task passes 171 tests; benchmark fixtures cover nonzero exits, signal termination, missing reports, hangs, and spawn failures; and [local development](local-dev.md) documents the validated root/Worker/browser commands. **Remaining**: exercise the complete CI workflow on GitHub, complete trusted advisory/update review, and decide whether best-effort link/performance workflows need promotion into a blocking release contract. **Validation**: exact task sequence, full benchmark gate, focused failure fixtures, report-path inspection, and Chromium smoke launch. **Rollback**: revert gate wiring as one change if any required check stops executing; retain the prior explicit validation command list.
...
- **S07-01 - Real reproducibility (Done; 2026-09-10 rehearsal).** Define the complete build inputs: commit, root and Worker lockfiles/toolchain snapshot, Node 24.21.0/npm 11.19.0, native Chromium/tooling versions, environment, and build mode. Two clean `npm run build` outputs from commit `0815b577` were compared with `SOURCE_DATE_EPOCH` fixed to the commit timestamp; `node scripts/check-reproducible.mjs --compare` reported matching content with digest `a4038eacad9dc35be8a9b49f910ba8261d4de1bded4ca262d205c4b88ecb3d7e`. The evidence was kept under `$env:TEMP/FamilyDashBoard/s07-r2`; no workspace artifacts remained. Missing input hashes, a dry run, or one manifest still fails to satisfy acceptance.
...
- **S08-01 - Data-flow and consent inventory (Done).** The inventory now covers direct, Worker, and public-proxy modes; error and performance telemetry; bearer-like calendar URLs; location; media; AI; and push subscriptions in [privacy](privacy.md), [data sources](data-sources.md), and [security](security.md). Public claims now identify recipients, triggers, defaults, retention, deletion, and logging behavior instead of describing public proxies as anonymous or telemetry as absent. Diagnostics, client error telemetry, support snapshots, browser-report D1 details, Worker request/error logs, and calendar cache keys redact or avoid URL query tokens; `worker-only` fails closed and `no-proxy` is enforced for calendar and Worker-aware fetches. Regression coverage exercises synthetic private tokens and the network-mode matrix. Manual provider terms/attribution review remains part of the broader S08 exit.
- **S08-02 - Provider and boundary review (Implementation complete; contract evidence remains maintained in provider docs).** Active news and calendar proxy boundaries now require HTTPS, reject URL credentials and non-default ports, match exact allowlisted hostnames, disable redirects, apply 8 s upstream timeouts, and cap RSS/ICS bodies at 1 MiB/2 MiB with explicit failure responses. R2 asset fetches use the same URL restrictions, disable redirects, and cap bodies at 10 MiB before caching. Regression coverage exercises rejected credentials/ports, calendar suffix-host attacks, redirects, oversized streamed/declared bodies, and body-read failures. Provider endpoints, cache TTLs, fallback behavior, and annual TTL review remain documented in [data sources](data-sources.md); calendar query tokens are excluded from Worker cache keys and are not echoed in errors. Manual provider terms/attribution review and diagnostic/export redaction evidence remain part of the broader S08 exit.
- **S08-03 - Operational controls (Done; 1 day).** [`worker/README.md`](../worker/README.md) now records request, upstream, response-memory, storage, telemetry, push, and inference budgets; alert signals; an operational owner; and fail-closed shutdown flags. Synthetic provider probes use fixed public targets, cache for five minutes, and fall back to an isolate-local snapshot when KV is unavailable or malformed, preventing repeated fan-out. Route-hit counters now prune at 30 days alongside seven-day latency samples, and AI synthesis uses its documented four-hour cache. The operational drill passes rate-limit, error-batch, report-truncation, probe-cache, retention, and optional-feature shutdown coverage. Stored optional data has documented deletion/expiry walkthroughs for errors, browser reports, telemetry, push subscriptions, AI, and probe caches.

**Validation**: targeted security tests, network/request inspection with synthetic data, redaction fixtures, provider-contract review, and 711 focused privacy/provider tests with root and Worker typechecks. **Rollback**: disable only the affected optional service through its existing control; retain safe local viewing. Privacy policy conflicts block E01 and E03-E05 expansion until resolved.
...
- **S09-03 - Financial trends (Done; 2-3 days).** Currency and stock histories use IndexedDB-backed hourly sampling with 168-point seven-day windows; currency daily snapshots retain 30 validated calendar entries. Rates are rendered as ILS per foreign unit with finite, positive-value validation, while stale cache renders are display-only and never extend trend history. Timestamp-aware sparklines leave visible breaks across missing observations and market closures instead of interpolating, and stale currency/stock indicators are visible and announced without replacing the accessible numeric tiles. Focused history/currency/stocks coverage passes 544 tests.
