...
> **Refresh date**: 2026-05-01 · **Shipped baseline**: v13.18.0 (Sprint 169). Active stream: v14.0 (Q1 2027) — V14-FOUNDATIONS, V14-SEMANTIC, V14-RESILIENCE. Roadmap items #7, #8, #10 confirmed Done.
>
> **Inventory**: 5155 tests / 162 suites / 0 failures · 0 ESLint errors · 0 ESLint warnings · 0 `eslint-disable` · 0 `@ts-ignore` · 0 TS errors · 0 markdownlint / stylelint issues · 45 ADRs · 0 client runtime deps · 2 worker deps (Hono + Valibot) · 6 themes · 12 cards · 4-tier offline cache.
>
...
| 3   | Refactor | Replace ad-hoc date math with Temporal in `hebrew-cal`/`calendar`/`countdown` once polyfill ≤ 10 KB | P1  | M   | Mid | v14.x   | Gate by polyfill size. Sprint 166: `scripts/check-temporal-polyfill-size.mjs` created. Gate CLOSED — `@js-temporal/polyfill` not yet installed; re-audit each quarter. |
...
| 8   | ~~Enhance~~ | ~~Coverage ratchet 89/81/89/90 → 92/85/92/93~~ | P0  | M   | Mid | ~~v14.0~~ **Done** | ✅ Sprint 143 (v13.15.0): thresholds raised to 93.5/85.0/92.5/94.7 — all targets exceeded. Actuals: 93.62/85.40/92.63/94.87. Sprint 167: branches ratcheted 85.5 → 85.7 (+18 tests: tasks ArrowDown/ArrowUp, signals peek-dirty, document-pip insertBefore). Actuals: 93.82/85.75/92.76/95.08. |
...
| 19  | Enhance  | LHCI perf threshold `warn 0.70` → `error 0.97` once Early Hints + SRI ship and CI runner stable     | P1  | S   | Mid | v14.x   | Sprint 147: ratcheted warn 0.70→0.80 (Sprint 124) → **0.85** (v13.16.0). Sprint 161: ADR-042 step 2 — promoted `warn 0.85` → **`error 0.85`** (HTTP Early Hints trigger met). Final `error 0.97` at v14.x. |
...
| 22  | Enhance  | Document Picture-in-Picture for video-news (gated: 3+ user requests)                                | P2  | S   | Lo  | v14.x   | Sprint 165: ADR-045 created + `docs/video-cards.md` PiP section added. Gated — request counter at 0/3. |
...
| 25  | Refactor | Narrow CSP `https://*.intel.com` wildcard to specific corp hosts when alternatives surface          | P2  | S   | Lo  | v14.x   | ADR-041 Sprint 133. Sprint 163 (Q2 2026): quarterly review — 0 new subdomains, 1/2 stable quarters toward Phase 2. Phase 2 unlocks after Q3 2026 review. |
| 26  | Enhance  | OWASP Top 10 audit (rotate per major release)                                                       | P0  | M   | Hi  | v14.0   | Sprint 160: patch-cycle smoke check for v13.17.0 complete — all 10 OWASP items re-verified (commit 49df655). Full v14.0.0 audit pending. Checklist in `.github/instructions/security-audit.instructions.md`. |
