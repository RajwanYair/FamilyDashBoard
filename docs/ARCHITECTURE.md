# FamilyDashBoard — Architecture (v15.7.0)
...
| Test framework   | **Vitest 4.1.7 + happy-dom 20**                                                                                                              | Vite-native, real DOM simulation, canonical gate via `npm run check` |
| Lint             | **ESLint 9 + typescript-eslint 8**                                                                                                           | Flat config, type-aware rules, 0 errors / 0 warnings enforced        |
...
| npm model        | Repository-local development-tool lockfile; parent **`MyScripts/`** remains a supported shared install; configs are vendored into `tooling/` | Standalone checkout or shared sibling-workspace setup                |
